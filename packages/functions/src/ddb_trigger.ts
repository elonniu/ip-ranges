...
export async function handler(event: any) {
    ...
    for (const record of Records) {
        ...
        if (eventName === 'MODIFY') {
            update[OldImage.name + '_' + OldImage.key] = `${OldImage.value.toLocaleString('en-US')} -> ${NewImage.value.toLocaleString('en-US')} (${NewImage.value - OldImage.value})`;
        } else if (eventName === 'INSERT') {
            update[NewImage.name + '_' + NewImage.key] = NewImage.value.toLocaleString('en-US') + ' new';
        } else if (eventName === 'REMOVE') {
            update[OldImage.name + '_' + OldImage.key] = OldImage.value.toLocaleString('en-US') + ' remove';
        }
    }
